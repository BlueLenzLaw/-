...
R_variable['type']='cifar_resnet_relu'
R_variable['seed']=0
...
# 模型训练测试的数据集设置
data = CIFAR()
R_variable['train_inputs'] = data.train_data[0:2000,:,:,:]
# print(R_variable['train_inputs'].shape)
R_variable['y_true_train'] = data.train_labels[0:2000]
R_variable['test_inputs'] = data.test_data[0:2000,:,:,:]
R_variable['y_true_test'] = data.test_labels[0:2000]
...
class model():
    def __init__(self):
        ...
        def inference(images, training, filters, n, ver):
            ...
            print('###')
            print(inputs.shape)
            inputs = tf.reshape(inputs, [-1,8*8*64])
            print('###')
            print(inputs.shape)
