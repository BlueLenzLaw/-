...
R_variable['actfun']='relu'###  0: ReLU; 1: Tanh; 3:sin;4: x**5,, 5: sigmoid  6 sigmoid derivate 7.sRelu 激活函数，可供选择
...
class model():
    def __init__(self):
        ...
        self.model.add(Conv2D(32, kernel_size=(3, 3),
                        input_shape=R_variable['graph_shape'],kernel_initializer=keras.initializers.TruncatedNormal(mean=0.0, stddev=0.1, seed=0)))
        self.model.add(Activation(R_variable['actfun']))
        # self.model.add(Activation(custom_activation, name='SpecialActivation1'))
        self.model.add(MaxPooling2D(pool_size=(2, 2)))
        self.model.add(Dropout(0.2))

        self.model.add(Conv2D(64, kernel_size=(3, 3),kernel_initializer=keras.initializers.TruncatedNormal(mean=0.0, stddev=0.1, seed=1)))
        self.model.add(Activation(R_variable['actfun']))
        # self.model.add(Activation(custom_activation, name='SpecialActivation2'))
        self.model.add(MaxPooling2D(pool_size=(2, 2)))
        self.model.add(Dropout(0.2))
        
        self.model.add(Flatten())

        self.model.add(Dense(512, kernel_initializer=keras.initializers.TruncatedNormal(mean=0.0, stddev=0.1, seed=2)))
        self.model.add(Activation(R_variable['actfun']))
        # self.model.add(Activation(custom_activation, name='SpecialActivation3'))

        self.model.add(Dense(64, kernel_initializer=keras.initializers.TruncatedNormal(mean=0.0, stddev=0.1, seed=3)))
        self.model.add(Activation(R_variable['actfun']))
        # self.model.add(Activation(custom_activation, name='SpecialActivation4'))

        self.model.add(Dense(10, kernel_initializer=keras.initializers.TruncatedNormal(mean=0.0, stddev=0.1, seed=4)))
