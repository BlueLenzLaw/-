...
R_variable['type']='cifar_cnn_relu'
R_variable['seed']=0
# np.random.seed(R_variable['seed'])
tf.set_random_seed(R_variable['seed'])
R_variable['ActFuc']='relu'   ###  0: ReLU; 1: Tanh; 2.sRelu 激活函数，可供选择
...
# 模型训练测试的数据集设置
data = CIFAR()
R_variable['train_inputs'] = data.train_data[:20000,:,:,:]
R_variable['y_true_train'] = data.train_labels[:20000]
...
class model():
    def __init__(self):
        ...
        # 卷积层
        # 3*3 conv layer, 
        W_convl = weight_variable([3, 3, 3, 32], name='W_conv1') 
        # sess1=tf.Session()
        # sess1.run(tf.global_variables_initializer())
        # a = W_convl.eval(session=sess1)
        # print('weight1:', a)
        b_convl = bias_variable([32], name = 'b_conv1')
        h_convl = activation_fun(conv2d(self.x, W_convl) + b_convl, name='h_conv1')
        h_pool1 = max_pool_2x2(h_convl, name='h_pool1')

        # 卷积层
        # 3*3 conv layer, input: 1 channel, output: 32 channels
        W_conv2 = weight_variable([3, 3, 32, 64], name='W_conv2') 
        b_conv2 = bias_variable([64], name='b_conv2')
        h_conv2 = activation_fun(conv2d(h_pool1, W_conv2) + b_conv2, name='h_conv2')
        h_pool2 = max_pool_2x2(h_conv2, name='h_pool2')

        # 全连接层
        h_pool2_flat = tf.reshape(h_pool2, [-1, R_variable['graph_shape'][0]*R_variable['graph_shape'][1]//16*64], name='h_pool2_flat')
        W_fc1 = weight_variable([R_variable['graph_shape'][1]*R_variable['graph_shape'][0]//16*64, 512], name = 'W_fc1')
        b_fc1 = bias_variable([512], name = 'b_fc1')
        h_fc1 = activation_fun(tf.matmul(h_pool2_flat, W_fc1) + b_fc1, name='h_fc1')

        # 全连接层
        W_fc2 = weight_variable([512, 64], name = 'W_fc2')
        ...
        y_pre = tf.add(tf.matmul(h_fc2, W_fc3), b_fc3, name='y_pre')
